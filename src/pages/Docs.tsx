...
const DocsPage = () => {
  useSEO({
    title: "Documentation",
    description: "Official developer documentation and API reference for the E-VARA platform.",
    ...
    <div className="min-h-screen bg-[#050608] text-white selection:bg-primary/30 font-mono">
      ...
      <div className="container mx-auto px-6 py-24 flex flex-col md:flex-row gap-12">
        ...
        <aside className="w-full md:w-64 shrink-0 space-y-6">
          <div>
            <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3">Getting Started</h4>
            <ul className="space-y-2 text-sm text-white/80">
              <li className="text-primary cursor-pointer hover:underline">Introduction</li>
              <li className="cursor-pointer hover:text-white transition-colors">Authentication</li>
              <li className="cursor-pointer hover:text-white transition-colors">Adding Identities</li>
            </ul>
          </div>
          <div>
            <h4 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3">Core Concepts</h4>
            <ul className="space-y-2 text-sm text-white/80">
              <li className="cursor-pointer hover:text-white transition-colors">Threat Severity</li>
              <li className="cursor-pointer hover:text-white transition-colors">Report Generation</li>
              <li className="cursor-pointer hover:text-white transition-colors">RBAC & Permissions</li>
            </ul>
          </div>
        </aside>
        ...
        <main className="flex-1 max-w-4xl">
          ...
          <p className="text-muted-foreground font-body text-lg mb-12">
            Everything you need to deploy, configure, and scale E-VARA within your organizational infrastructure.
          </p>

          <div className="grid sm:grid-cols-2 gap-6">
            {sections.map((sec, i) => (
              <div key={i} className="p-6 rounded-2xl border border-white/5 bg-[#0A0C12] hover:border-primary/30 transition-all cursor-pointer group">
                <div className="flex items-center gap-3 mb-4">
                  <div className="p-2 bg-primary/10 rounded-lg group-hover:scale-110 transition-transform">
                    {sec.icon}
                  </div>
                  <h3 className="font-bold text-sm uppercase tracking-widest">{sec.title}</h3>
                </div>
                <p className="text-xs text-muted-foreground font-body leading-relaxed">{sec.desc}</p>
              </div>
            ))}
          </div>
        </main>
